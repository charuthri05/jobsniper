...
8. PRESERVATION CHECK: Every experience sub-section (current and previous roles) may receive additions and light rewording. Flag any proposed change that (a) changes a bullet's underlying fact, technology, or what the metric measured; (b) changes or removes a metric number; (c) adds soft-skill claims or filler ("scalable", "production-ready", "mentoring", "leading", "driving") not in the source; (d) merges, splits, drops, or reorders a bullet that exists in the master template; (e) removes a whole experience sub-section.
9. PAGE-FIT CHECK: If the plan proposes dropping a whole experience section, flag it. If the plan proposes removing, merging, or splitting a master-template bullet, flag it. For page overflow, the allowed compression order is: drop whole projects first (lowest-signal first), then skill category lines, then as a last resort remove an ADDED experience bullet (never a master-template bullet). Flag deviations from this order.
