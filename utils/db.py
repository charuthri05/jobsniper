...
def get_jobs_by_status(status: str) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM jobs WHERE status = ? ORDER BY CASE WHEN source = 'manual' THEN 0 ELSE 1 END, score DESC, date_scraped DESC",
            (status,),
        ).fetchall()
...
def get_queued_without_cl() -> list[dict]:
    ...
    try:
        rows = conn.execute(
            ...
            "ORDER BY CASE WHEN source = 'manual' THEN 0 ELSE 1 END, score DESC, date_scraped DESC"
        ).fetchall()
        return [dict(row) for row in rows]
    finally:
        conn.close()
