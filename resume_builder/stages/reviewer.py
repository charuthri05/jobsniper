...
6. STYLE: Flag any proposed rewrites that (a) use em-dashes ("---", "--", " — ") inside bullet text, (b) narrate internal architecture instead of claiming impact, (c) use filler words ("leveraging", "utilizing", "hands-on", "proactively", "materially", "significantly improved", "cutting-edge", "robust", "seamless", "enterprise-grade", "production-ready"), (d) exceed ~1.5 lines per bullet, or (e) break from the sentence rhythm of the candidate's existing bullets in the template.
7. FABRICATION CHECK: Flag any proposed metric that is not present in the source materials. "reduced incident response time by 60%" is a fabrication if the source only says "reduced incident response time". Flag any soft claim (mentoring, leading, driving cross-functional discussions) not explicitly stated in the source materials. Flag any fabricated uptime figures, dollar amounts, or team-size claims.
8. ADDITIONS CHECK (NEW — CRITICAL): Verify the plan actually proposes JD-relevant additions. A plan that has NO CURRENT_ROLE_ADDITIONS and NO PREVIOUS_ROLE_ADDITIONS is almost certainly a failure of tailoring, unless the master template happens to already surface every single JD priority (rare). Cross-reference the JD_PRIORITIES against the master template bullets. For every JD priority that the master does not surface but the candidate's source materials do describe, flag it as a missing addition.
9. PRESERVATION CHECK: Every experience sub-section may receive additions and light rewording. Flag any proposed change that (a) changes a bullet's underlying fact, technology, or what the metric measured; (b) changes or removes a metric number; (c) adds soft-skill claims or filler ("scalable", "production-ready", "mentoring", "leading", "driving") not in the source; (d) merges or splits a bullet that exists in the master template; (e) removes a whole experience sub-section.
10. PAGE-FIT CHECK: Master bullets should be kept by default. Flag any proposed drop of a master bullet unless (a) the resume would exceed two pages after all additions and (b) the bullet is among the lowest-JD-relevance master bullets. The allowed compression order is: drop whole projects first (lowest-signal first), then drop lowest-JD-relevance master bullets, then drop skill category lines, then as a last resort trim an ADDED bullet. Flag deviations from this order. Flag any plan that drops a JD-relevant bullet or a whole experience sub-section.
